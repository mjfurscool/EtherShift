...
addAnimationOverride = function(enemyId, originalAnimation, newAnimation){
  if (!animationOverrides[enemyId]) {
    animationOverrides[enemyId] = [];
  }
  animationOverrides[enemyId][originalAnimation] = newAnimation;
};



getAnimationOverride = function(self, targets, item) { //Called by QABSManager.startAction -- look in TerraxLightingQABS
  if (self == $gamePlayer && targets[0]) {
    if (!targets[0]._battler) {
      return;
    }
    a = animationOverrides[targets[0]._battler._enemyId];
    if (a) {
      return a[item.data.animationId]
    }
  }
};

addAnimationOverride(17, 123, 149); //Pavise, bullet hit, bullet hit shield
addAnimationOverride(10, 123, 149);// sword on Sentry10
addAnimationOverride(11, 123, 149);// sword on Sentry11
addAnimationOverride(8, 123, 150);// sword on Sentry10
addAnimationOverride(14, 123, 147);// sword on shield_golem1
addAnimationOverride(15, 123, 150);// sword on non_shield_golem1
addAnimationOverride(19, 123, 147);// sword on shield_golem2
addAnimationOverride(20, 123, 150);// sword on non_shield_golem2
addAnimationOverride(5, 123, 148);// sword on evo

addAnimationOverride(10, 122, 153);//bullet on sentry10
addAnimationOverride(11, 122, 153);//bullet on sentry11
addAnimationOverride(8, 122, 153);//bullet on golem
addAnimationOverride(15, 122, 153);//bullet on non_shield_golem
addAnimationOverride(20, 122, 153);//bullet on non_shield_golem2

addAnimationOverride(14, 122, 157);//bullet on shield_golem
addAnimationOverride(19, 122, 157);//bullet on shield_golem2

addAnimationOverride(5, 122, 154);//bullet on evo

addAnimationOverride(10, 124, 155);//laser on sentry10
addAnimationOverride(11, 124, 155);//laser on sentry11
addAnimationOverride(8, 124, 155);//laser on golem
addAnimationOverride(15, 124, 155);//laser on non_shield_golem
addAnimationOverride(20, 124, 155);//laser on non_shield_golem2

addAnimationOverride(14, 124, 157);// laser on shield_golem
addAnimationOverride(19, 124, 157);//laser on shield_golem2

addAnimationOverride(5, 124, 156);//laser on evo

addAnimationOverride(17, 122, 157);// bullet on pavise
addAnimationOverride(17, 124, 157);//laser on pavise
//-----------------------------------------------------------------------------
// DataManager
//
...
flashes.init = function(){
  this.sprites = {};
  this.watchedFlashes = [];
  this.meoStates = [31]; //these aren't used
  this.wingStates = [32]; //these aren't used
  this.meoCounters = [14, 17, 19]; //these are enemy ids
  this.wingCounters = [8, 15, 20];
};

flashes.isMeoState = function(id){
  return id == 31;
};

flashes.isMeoCounter = function(enemyId){
  return this.meoCounters.indexOf(enemyId) != -1;
};

flashes.isWingState = function(id){
  return id == 32;
};

flashes.isWingCounter = function(enemyId){
  return this.wingCounters.indexOf(enemyId) != -1;
};

flashes.addSprite = function(sprite){
  id = sprite._character._eventId
  this.sprites[id] = sprite;
  //sprite._blendColor = [255, 0, 0, 255]; //just to check
};

flashes.update = function(){
  newWatched = [];
  for (i = 0; i < this.watchedFlashes.length; i++) {
    id = this.watchedFlashes[i];
    sprite = this.sprites[id];
    bc = sprite._blendColor;
    //deal with blend color
    v = bc[3];
    v -= 4;
    if (v < 0) {
      v = 0;
    ...
    bc[3] = v;
    sprite._blendColor = bc;
    //done with blend color
    sprite._refresh();
  }
  this.watchedFlashes = newWatched;
};

flashes.clearSprites = function(){
  this.sprites = {};
  this.watchedFlashes = [];
};

flashes.stateRemoved = function(stateId, eventId, enemyId){
  if (flashes.isMeoState(stateId)) {
    sprite = flashes.sprites[eventId];
    flashes.watchedFlashes.push(eventId);
    if (flashes.isMeoCounter(enemyId)) {
      sprite._blendColor = [0, 255, 255, 255];
    } else {
      sprite._blendColor = [255, 0, 0, 255];
    }
    sprite._refresh();
  } else if (flashes.isWingState(stateId)) {
      sprite = flashes.sprites[eventId];
      flashes.watchedFlashes.push(eventId);
      if (flashes.isWingCounter(enemyId)) {
        sprite._blendColor = [0, 255, 255, 255];
      } else {
        sprite._blendColor = [255, 0, 0, 255];
      }
...
flashes.init(); //call this for now
///


remapSkillkey = function(keyNumber, skillId){
  $gameSystem._absKeys[keyNumber].skillId = skillId;
};


clearDisabledSkills = function(){
  $disabledSkills = [];
};


skillIsDisabled = function(id){
...
reenableSkill = function(id){
  var newArr = [];
  var len = $disabledSkills.length;

  for (var xd = 0; xd < len; xd++) {
    if (id == $disabledSkills[xd]) {
      continue;
    }

    newArr.push($disabledSkills[xd]);
  }

  $disabledSkills = newArr;
};


addBullets = function(val, addToMag){
  maxReserve = $gameVariables.value(6)
  reserve = $gameVariables.value(5);
  tp = $gameVariables.value(4);
  mag = $gameVariables.value(7);



  if (addToMag){
    for (var xd = val; xd > 0; xd--) {
      if (tp < mag) {
        tp += 1;
        continue;
      } else if (reserve < maxReserve) {
        reserve += 1;
      } else {
        break;
      }
  ...
  $gameVariables.setValue(4, tp);
  $gameVariables.setValue(5, reserve);

};


regenMeoAmmo = function(){
  maxReserve = $gameVariables.value(6)
  reserve = $gameVariables.value(5);
  tp = $gameVariables.value(4);
  mag = $gameVariables.value(7);
  ...
  if(tp < mag){
  	$gameVariables.setValue(4, tp+1);
    return
  }
  if(reserve < maxReserve){
    $gameVariables.setValue(5, reserve+1);
  }
};


clearGolemTransitions = function() {
  $golemList = {};
  $golemList["keys"] = [];
};

addGolemTransition = function(event, newId) {
  var eventId = event._eventId;
  $golemList["keys"].push(eventId);
  $golemList[eventId] = [Date.now(), newId];
  event._isGolem = true;
};

runGolemTransitions = function(minFrames){
  var keys = $golemList["keys"];
  var le = keys.length;
  var minTime = minFrames*16; //Approximate number of frames in milliseconds
  var curTime = Date.now();

  for (var xd = 0; xd < le; xd++) {
    var key = keys[xd];
    if (key == -1) {
      continue;
    }

    var info = $golemList[key];
    if (curTime - info[0] < minTime) {
      continue;
    }

    $gameMap._events[key]._battlerId = info[1];
    $gameMap._events[key]._isDead = false;
    delete $gameMap._events[key]._noProjectiles;
    $gameMap._events[key].setupFakeBattler(info[1]);
...
permadeath.add = function(event){
  if (!$pdeathList[event._mapId]) {
    $pdeathList[event._mapId] = [];
  }
  rx = Math.round(event._realX*10)/10
  ry = Math.round(event._realY*10)/10

  $pdeathList[event._mapId].push([event._eventId, rx, ry]);

};

permadeath.init = function(){
  $usePdeath = true;
};

permadeath.apply = function(eventList){
  removeList = $pdeathList[$curMap];
  if (!removeList) {
    return;
  }
  var rLen = removeList.length;
  for (xd = 0; xd < rLen; xd++) {
    rm = removeList[xd];

    eventId = rm[0];

    eventList[eventId].note = ""; //clear enemy tag
    eventList[eventId].x = rm[1];
    eventList[eventId].y = rm[2];
  }
};


//end permadeath stuff



addToDeadList = function(mapId, eventId, flag){
  if($deadEnemiesList[mapId] == undefined){
    $deadEnemiesList[mapId] = new Array();;
  }

  $deadEnemiesList[mapId].push([eventId, flag]);
};
...
DataManager.loadMapData = function(mapId) {
  ...
  //Clear golem transitions

  if (oldMap != mapId) {
	QABSManager.clear();
    flashes.clearSprites();
    var clearList = $deadEnemiesList[mapId] || [];
    var clearLen = clearList.length || 0;

    for (var xd = 0; xd < clearLen; xd++) {
      var eId = clearList[xd][0];
      var flag = clearList[xd][1];

      $gameSelfSwitches.setValue([mapId, eId, flag], false);
    }

    if ($deadEnemiesList[mapId]) {
      delete $deadEnemiesList[mapId];
    }

    clearGolemTransitions();
  }
  //console.log("Should be undefined: " + $deadEnemiesList[mapId]);
...
DataManager.makeSaveContents = function() {
    ...
    contents.curMap = $curMap;
    contents.usePdeath = $usePdeath;
    contents.pdeathList = $pdeathList;
    contents.disabledSkills = $disabledSkills;
    return contents;
};
...
DataManager.extractSaveContents = function(contents) {
    ...
    $curMap = contents.curMap;
    $usePdeath = contents.usePdeath;
    $pdeathList = contents.pdeathList;
    $disabledSkills = contents.disabledSkills;
};
...
SceneManager.updateScene = function() {
  flashes.update(); //added to update flashes every frame
    if (this._scene) {
