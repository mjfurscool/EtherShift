 ...
 *    - 4: Stops when it hits the player or tile (new)
 * - #### throughTerrain:
...
(function() {
  ...
  Skill_Sequencer.prototype.startUserAction = function(action) {
    ...
    switch (cmd) {
      ...
      case 'smartteleport': {
        this.userSmartTeleport(action);
        break;
      }
      case 'wingblink':{
        var resolution = Number(action[0]);
        var successID = Number(action[1]);
        var failureID = Number(action[2]);
        var costVar = Number(action[3]);

        var startTime = Date.now();
        var succeeded = this.wingBlink(resolution);
        $teleportTime += Date.now() - startTime;
        console.log("Teleporting took: " + (100 * $teleportTime / (1000/60)) + "% of a frame");
        if (!succeeded) {
          this.userForceSkill([failureID]);
        } else {
          this._character.battler()._mp -= $gameVariables.value(costVar);
          this.userForceSkill([successID]);
        }
        break;
      }
      case 'jump': {
  ...
  Skill_Sequencer.prototype.userSpeed = function(action) {
    ...
    if (action[0] === 'inc') {
      ...
      this._character.setMoveSpeed(Number(action[1]), 1);
    }
  };
  ...
  //Used for accurateMove, tests if actor can reach the point
  Skill_Sequencer.prototype.testNode = function(action, dist, radian){
    var dir = this._character.radianToDirection(radian, true);
    ...
    var x2 = $gameMap.roundPXWithDirection(this._character._px, horz, horzSteps);
    var y2 = $gameMap.roundPYWithDirection(this._character._py, vert, vertSteps);
    return (this._character.canPassToFrom(x2, y2, this._px, this._py));
  }

  //Lets the user move less than the specified distance
  //Must specify a resolution
  Skill_Sequencer.prototype.userAccurateMove = function(action) {
    var dist = Number(action[1]) || this._character.moveTiles();
    var route = {
      list: [],
      repeat: false,
    ...
    var resolution = Number(action[3]);
    var reached = false;
    console.log(this.testNode(action, dist, radian))

    stepDist = dist/resolution;
    for (var xd = 0; xd < resolution; xd++) {
      testDist = dist - (stepDist * xd);
      if (this.testNode(action, testDist, radian)) {
        dist = testDist;
        reached = true;
        break;
      }
  ...
  Skill_Sequencer.prototype.userSmartTeleport = function(action) {
    var x1 = this._skill.collider.x;
    var y1 = this._skill.collider.y;

    var resolution = Number(action[0]);
    var origX = this._character._px;
    var origY = this._character._py;

    var xStep = (x1 - origX) / resolution;
    var yStep = (y1 - origY) / resolution;

    for (var xd = 0; xd < resolution; xd++) {
      var newX = x1 - (xStep * xd);
      var newY = y1 - (yStep * xd);
      if(this._character.smartTeleportCheck(newX, newY, 'collision')){
        this._character.setPixelPosition(newX, newY);
        break;
      }
    }
  ...
  Skill_Sequencer.prototype.wingBlink = function(resolution) {
    var x1 = this._skill.collider.x;
    var y1 = this._skill.collider.y;
    ...
    for (var xd = 0; xd < resolution; xd++) {
      ...
      if(this._character.smartTeleportCheck(newX, newY, 'collision')){
        this._character.setPixelPosition(newX, newY);
        this._character._lastX = origX;
        this._character._lastY = origY;
        return true;
      }
    }
  ...
  Skill_Sequencer.prototype.fastActionMove = function(action) {
    var dir = action[0];
    var distance = Number(action[1]);
    var resolution = Number(action[2]);

    distance = distance/resolution;

    ColliderManager.draw(this._skill.collider, 1);
    var radian = this._skill.radian;
    if (dir === 'backward') {
      radian -= Math.PI;
    }
    ...
    this._waitForMove = action[3] === 'true';
    this.setSkillRadian(Number(radian));
    this.fastActionMoveSkill(distance, resolution);
  };
  ...
  Skill_Sequencer.prototype.fastActionMoveSkill = function(distance, resolution) { //actually move the skill
    $teleportTime = 0;
    var startTime = Date.now();
    ...
    for (var xd = 0; xd < resolution; xd++) {
      this._skill.newX = this._skill.collider.x + Math.round(distance * Math.cos(this._skill.radian));
      this._skill.newY = this._skill.collider.y + Math.round(distance * Math.sin(this._skill.radian));
      this._skill.speed = Math.abs(distance / 1);
      this._skill.speedX = Math.abs(this._skill.speed * Math.cos(this._skill.radian));
      this._skill.speedY = Math.abs(this._skill.speed * Math.sin(this._skill.radian));
      this._skill.moving = true;
      this.fastUpdateSkillPosition();
  ...
  Skill_Sequencer.prototype.targetJump = function(action, targets, recursionDepth, distStep) {
    if (recursionDepth == null) {
      recursionDepth = 5;
      distStep = Number(action[1])/(recursionDepth + 1);
    ...
    for (var i = 0; i < targets.length; i++) {
      ...
      if (dx == 0 && dy == 0) {
        recurseList.push(targets[i]);
      }
    }
    if (recurseList.length > 0) {
      action[1] = (Number(action[1]) - distStep).toString();
      this.targetJump(action, recurseList, recursionDepth - 1, distStep);
    }
  };
  ...
  Skill_Sequencer.prototype.canSkillMove = function() {
    ...
    if (targets.length > 0) {
      ...
      if (targets.length > 0) {
        ...
        if (through === 1 || through === 3 || through === 4) {
          collided = true;
          // TODO select the nearest target
          this._skill.targets = [targets[0]];
        } else if (through === 4) {
          if (targets.indexOf($gamePlayer) != -1) {
            collided = true;
          }
        }
        this.updateSkillDamage();
    ...
    if (through === 2 || through === 3 || through === 4) {
  ...
  Skill_Sequencer.prototype.fastUpdateSkillPosition = function() {
    if (this._skill.waving) {
      return this.updateSkillWavePosition();
    }
    var collider = this._skill.collider;
...
(function() {
  ...
  Game_Interpreter.prototype.qABSCommand = function(args) {
    ...
    if (cmd === 'force') { //added for character swapping
        id = Number(args[0]);
      $charRef.userForceSkill([id]);
    }
  };
})();
...
(function() {
  ...
  Game_Battler.prototype.removeState = function(stateId) {
    ...
    flashes.stateRemoved(stateId, this._charaId, this._enemyId);
...
(function() {
  ...
  Game_Enemy.prototype.setup = function(enemyId, x, y) {
    ...
    this._aiSR = (meta.AISR || 3);
    this._aiSA = (meta.AISA || 0);

    if (!meta.noPathfind){ //CHANGED
      this._noPathfind = 0;
    } else{
      this._noPathfind = 1;
    }
...
(function() {
  ...
  Game_CharacterBase.prototype.clearABS2 = function() {
    if (this._activeSkills && this._activeSkills.length > 0) {
      this.clearSkills();
    }
    if (this._agroList) {
  ...
  Game_CharacterBase.prototype.canUseSkill = function(id) {
    ...
    if (this.isHumanPlayer) {
      return this.usableSkills().contains(id) && this.battler().canPaySkillCost(skill) && !skillIsDisabled(id);
    }
  ...
  Game_CharacterBase.prototype.updateABS = function() {
    if (this.battler().isDead()) {
      if (this._noProjectiles) {
        return;
      }

      if (!this._isDead) {
        this.onDeath();
        this.battler()._hideHpBar = true;
      }
      if (!this.updateSkillsDead()){
        if (!this._isGolem) {
          this.clearABS();
          this._noProjectiles = true;
        }
      }
  ...
  Game_CharacterBase.prototype.updateSkillsDead = function() {
    var ret = false;
    if (this._groundTargeting) this.updateTargeting();
    if (this._activeSkills.length > 0) ret = true;
    if (ret) {
...
(function() {
  ...
  Game_Event.prototype.setupBattler = function() {
    ...
    if (foe) {
      ...
      this._aiSR = this._battler._aiSR * 50;
      this._aiSA = this._battler._aiSA * 50;
      this._aiRange = this._battler._aiRange || QABS.aiLength;
      this._aiWait = 0;
      this._aiPathfind = Imported.QPathfind && QABS.aiPathfind && (this._battler._noPathfind == 0); //CHANGED
  ...
  Game_Event.prototype.setupFakeBattler = function(id) {
    delete this.STOPMOVING;
    var foe = /<enemy:([0-9]*?)>/i.exec(this.notes());
    if (foe) {
      this.clearABS();
  ...
  Game_Event.prototype.validAI = function() {
    ...
    // updateAI will run
    return this._aiType === "simple" || this._aiType === "ranged";
  };
  ...
  Game_Event.prototype.updateAI = function(type) {
    if (type === 'simple') {
      ...
      return this.updateAIranged();
    }
    // to add more AI types, alias this function
    // and do something similar to above
  };

  Game_Event.prototype.updateAIranged = function() {
    var bestTarget = this.bestTarget();
    if (!bestTarget) return;
    var targetId = bestTarget.charaId();
    if (!this.AIRangedInRange(bestTarget)) return;
    this.AIRangedAction(bestTarget, this.AIRangedGetAction(bestTarget));
  };
  ...
  Game_Event.prototype.AIRangedInRange = function(bestTarget) {
    var targetId = bestTarget.charaId();
  ...
  Game_Event.prototype.AIRangedGetAction = function(bestTarget) {
    var bestAction = null;
  ...
  Game_Event.prototype.AIRangedAction = function(bestTarget, bestAction) {
    if (bestAction) {
      ...
      if (this._aiPathfind) {
        ...
        if (Math.abs(dx) <= mw && Math.abs(dy) <= mh) {
          ...
          if (Math.abs(dx) * Math.abs(dx) + Math.abs(dy) * Math.abs(dy) > this._aiSR * this._aiSR){
            this.initChase(bestTarget.charaId());
            //this.clearPathfind();
          }
          else if (Math.abs(dx) * Math.abs(dx) + Math.abs(dy) * Math.abs(dy) > this._aiSA * this._aiSA) {
            this.clearPathfind();
          }
          else {
            this.clearPathfind();
  ...
  Game_Event.prototype.endCombat = function() {
    ...
    if (this._aiPathfind || Imported.QPathfind) {
      ...
      if (this._battler._noWalkback == 0) { //CHANGED
      ...
      //this.findRespawnLocation(); //CHANGED
    }
    this.refresh();
  };
  ...
  Game_Event.prototype.onDeath = function() {
    this.STOPMOVING = true; //Stop pathfinding lmao
    if (this._onDeath) {
      try {
        eval(this._onDeath);
      } catch (e) {
