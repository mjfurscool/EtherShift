...
(function() {
  ...
  Game_Interpreter.prototype.qAudioCommand = function(args) {
    ...
    if (cmd === 'clearbgm') {
      return AudioManager.stopAllBGM();
    }
    if (cmd === 'save') {
    ...
    if (cmd === 'setvol') {
      ...
      var volume = Number(QPlus.getArg(args, /^vol(.+)/i) || '*');
      return AudioManager.setVolume(id, volume);
    }
    if (cmd === 'adjvol') {
      var id = QPlus.getArg(args, /^id(.+)/i) || '*';
      var volume = Number(QPlus.getArg(args, /^vol(.+)/i) || '*');
      return AudioManager.adjustVolume(id, volume);
    }
  };
  ...
  AudioManager.stopAllQAudio = function() {
    var buffers = this._QAudioBuffers;
    var qAudios = $gameSystem._QAudios;
    var nulled = 0;
    var newBuffers = [];
    var newQAudios = [];
    console.log(buffers);
    ...
    for (var i = 0; i < buffers.length; i++) {
      if (buffers[i].type == 'bgm') {
        newBuffers.push(buffers[i]);
        if (qAudios) {
          newQAudios.push(qAudios[i]);
          if (buffers[i].uid != qAudios[i]["id"]) {
            console.log("Report this to Taylor F in the Slack group: The AudioManager._QAudioBuffers[i] is not the same as the $gameSystem._QAudios[i]");
          }
        }
        continue; //Skip if is a bgm
      }
    ...
    console.log(buffers);
    this._QAudioBuffers = buffers;
    if ($gameSystem) {
      $gameSystem._QAudios = newQAudios;
    }
  };

  AudioManager.stopAllBGM = function() {
    var buffers = this._QAudioBuffers;
    var qAudios = $gameSystem._QAudios;
    var nulled = 0;
    var newBuffers = [];
    ...
    for (var i = 0; i < buffers.length; i++) {
      if (buffers[i].type !== 'bgm') {
        ...
        continue; //Skip if it's not a bgm
      }
      if (buffers[i]._stopListeners) {
        buffers[i]._stopListeners.length = 0;
      }
  ...
  AudioManager.setVolume = function(id, volume) {
    var buffers = this._QAudioBuffers;
    for (var xd = 0; xd < buffers.length; xd++) {
      if (buffers[xd].uid === id) {
        buffers[xd].rVolume = volume;
        break;
      }
    }
  };

  AudioManager.adjustVolume = function(id, volume) { //Added this for transitions
    var buffers = this._QAudioBuffers;
    for (var xd = 0; xd < buffers.length; xd++) {
      if (buffers[xd].uid === id) {
        var tempVol = buffers[xd].rVolume;
        tempVol += volume;
        var maxVolume = buffers[xd].maxVolume;

        if(tempVol < 0) {
          buffers[xd].rVolume = 0;
        } else if (tempVol > maxVolume) {
          buffers[xd].rVolume = maxVolume;
        } else {
          buffers[xd].rVolume = tempVol;
        }
        break;
      }
    }
