 ...
 *
 * tx_missile: 25, #FF0000
 * Will give the missile a small (radius 25) red (color #FF0000) lightsource
 *
 * tx_onhit: 30, #FFAA00, 10, FADEOUT, 5
 * Will give a small yellow lightflash whenever the missile impacts on a wall or flies out of view, it will fadeout on the last 5 frames
 *
 * tx_onhitNPC: 100, #FFAA00, 15
 * Will give a large yellow lightflash when the missile impacts with an NPC character.
 *
 *
 * Example :
 *
...
(function() {
	...
	// OVERWRITTEN FROM QUASI ABS TO ADD LIGHTSOURCES TO missileS AND END ANIMATIONS
	// Skill_Sequencer
	...
	Skill_Sequencer.prototype.update = function() {
		...
		if (this._skill.moving) {
			...
			Terrax_ABS_skill.push(this._skill.settings.tx_missile);
	...
	QABSManager.startAction = function(self, targets, item) {
		if (!item.animationTarget || targets.length === 0) {
			id = getAnimationOverride(self, targets, item) || item.data.animationId;
			this.startAnimation(id, item.collider.center.x, item.collider.center.y);
		}
